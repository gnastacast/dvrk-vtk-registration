...
class RegistrationObject(object) :
	...
	def __init__(self, stlFile, camData=None, bwImage=None, colorImage=None):
		...
		# If no image exists, generate one
			...
			self.bwImage,self.colorImage = self.generateImages(640/2,480/2)
		
		# Initialize interactor
		self.iren.SetRenderWindow(self.renWin)
		style = self._RegistrationInteractorStyle(self.camStatic,self.renWin)
	...
	def manualRegistration(self, inputTransform=None):
		...
		print("Annealed Pos: " + str(outputTransform.GetPosition()))
		print("Annealed Rot: " + str(outputTransform.GetOrientation()))
	...
	class _RegistrationInteractorStyle(vtk.vtkInteractorStyleTrackballCamera):
		...
		# Listen for keypress
		def __init__(self,camera,renWin,parent=None):
			#self.AddObserver("KeyPressEvent", self.keyPressEvent)
			self.renWin = renWin;
		'''
		HACK TO RESIZE WINDOW
		def keyPressEvent(self,obj,event):
			...
			if key=='c' or key =='C':
				...
				self.renWin.SetSize(320*3,240*3)
